...
const updateWorkflowSchema = z.object({
    stepName: z.enum(['jobDescription', 'resumeScreening', 'interviewPlanning', 'hiringDecision', 'offer']),
...
export async function PATCH(req: Request, { params }: { params: Promise<{ jobId: string }> }) {
    try {
        ...
        if (stepName) {
            ...
            if (data) {
                job.workflow.steps[stepName].data = { ...job.workflow.steps[stepName].data, ...data }
            }
        }
